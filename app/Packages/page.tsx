...
const Page: React.FC = () => {
  // Tipamos el estado como Package[] para guardar un array de `Package`.
      ...
      try {
        ...
        if (Array.isArray(rawData)) {
          ...
          const uniqueCategories = Array.from(
            new Set(rawData.map((item: Package) => item.category?.toLowerCase()))
          );
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== "all") {
      // Usamos toLowerCase() para asegurar consistencia
  ...
  return (
    <section className="bg-gray-100 pt-[170px] pb-20 px-4 sm:px-6 md:px-12 min-h-screen">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-12">
        {/* SIDEBAR - FILTROS */}
        <motion.div
          className="w-full md:w-1/4 bg-white p-6 rounded-xl shadow-lg border border-gray-200 flex-none"
        ...
        >
          {filteredData.map((item) => (
            ...
            >
              ...
              {/* Evitamos que el click del botón abra el Popup */}
