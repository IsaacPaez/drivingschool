              ...
              {weekDays.map((d, colIdx) => {
                ...
                if (block?.status === 'booked' || block?.status === 'scheduled') {
                  return (
                    <td
                      key={colIdx}
                      className="p-0 border text-center align-middle w-20 md:w-32"
                      style={{ background: '#0056b3', border: '1px solid #e0e0e0', borderRadius: 0, cursor: 'pointer', height: '22px', minHeight: '22px', minWidth: 60, maxWidth: 60 }}
                ...
                if (block?.status === 'cancelled') {
                  return (
                    <td
                      key={colIdx}
                      className="p-0 border text-center align-middle w-20 md:w-32"
                      style={{ background: '#f44336', border: '1px solid #e0e0e0', borderRadius: 0, height: '22px', minHeight: '22px', minWidth: 60, maxWidth: 60 }}
                    >
                      <div className="flex items-center justify-center w-full font-bold gap-1" style={{ borderRadius: 0, height: '22px', fontSize: '0.95rem', color: '#fff' }}>
                ...
                if (block?.status === 'free' || block?.status === 'available') {
                  return (
                    <td
                      key={colIdx}
                      className="p-0 border text-center align-middle w-20 md:w-32"
                      style={{ background: '#b0b0b0', border: '1px solid #e0e0e0', borderRadius: 0, height: '22px', minHeight: '22px', minWidth: 60, maxWidth: 60 }}
                    >
                      <div className="flex items-center justify-center w-full font-bold gap-1" style={{ borderRadius: 0, height: '22px', fontSize: '0.95rem', color: '#fff' }}>
